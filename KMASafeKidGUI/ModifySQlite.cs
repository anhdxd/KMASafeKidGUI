...
namespace KMASafeGUI
{
    class ModifySQlite
    {
        ...
        public static string PATH_DB = Directory.GetCurrentDirectory() + "\\" + NAME_DB_SQLITE;
        ...
        { 
            try
            {
                ...
                command.CommandText = "CREATE TABLE IF NOT EXISTS tb_Blw(ObjectName TEXT UNIQUE, PathApp TEXT, ObjectType INTEGER)";
        ...
        public bool DeleteAppFromDB(string sPathApp)
        {
            ...
            try
            {
                ...
                command.CommandText = string.Format("DELETE FROM tb_Blw WHERE PathApp = \"{0}\"", sPathApp);
                command.ExecuteNonQuery();
            }
            catch (SQLiteException ex)
            {
        ...
        public static SortedSet<string> GetAllBlockRule(ObjectType objectType)
        {
            ...
            try
            {
                sQ.ConnectionString = String.Format("Data Source = {0}\\{1}", Directory.GetCurrentDirectory(), NAME_DB_SQLITE);
                sQ.Open();
